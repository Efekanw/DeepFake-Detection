...
def visionTransformerPredict(file_path):
    ...
    result_predict = run.main(model_path="cross__efficient__vit/cross_efficient_vit.pth", config="cross__efficient__vit/configs/architecture.yaml",file_path=file_path)
    return str(result_predict)


def visionTransformerTrain(num_epochs, train_path, val_path, test_path, training_params, model_name, patience=5, resume="cross__efficient__vit/cross_efficient_vit.pth"):
    ...
    result_test = test.main(model_name=model_name, model_path=model_path, batch_size=training_params["bs"], config="cross__efficient__vit/configs/architecture.yaml")
    return str(result_test)